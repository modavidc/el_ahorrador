...
class YapeTransaction {
  ...
  final String?
  description; // Agregar campo para la descripción del producto/servicio
...
class YapeParser {
  ...
  static final _securityCodePattern = RegExp(
    r'código de seguridad[^\d]*(\d{3})',
    caseSensitive: false,
  );
  static final _operationNumberPattern = RegExp(
    r'nro\.?\s*de\s*operación[^\d]*(\d+)',
    caseSensitive: false,
  );
  static final _phonePattern = RegExp(
    r'nro\.?\s*de\s*celular[^\d]*(\d{3})\s*(\d{3})\s*(\d{3})',
    caseSensitive: false,
  );

  // Patrones adicionales para el formato específico de la captura
  static final _dateTimePattern = RegExp(
    r'(\d{1,2})\s*(ene|feb|mar|abr|may|jun|jul|ago|sep|oct|nov|dic)\.?\s*(\d{4})\s*\|\s*(\d{1,2}):(\d{2})\s*(a\.?\s*m\.?|p\.?\s*m\.?)',
    caseSensitive: false,
  );
  ...
  static Future<ParsedExpense> toExpense(
    YapeTransaction yapeTransaction, {
    ...
    final subcategory = await _getSubcategory(
      category,
      yapeTransaction.recipient,
    );

    // Usar la descripción real de la captura (Nota) o generar una por defecto
    final description =
        yapeTransaction.description ?? 'Pago a ${yapeTransaction.recipient}';
    ...
    return ParsedExpense(
      ...
      destination:
          yapeTransaction.recipient, // Para gastos, destination es el receptor
  ...
  static Future<String> _categorizeByRecipient(String recipient) async {
    ...
    if (recipientLower.contains('taxi') ||
        recipientLower.contains('uber') ||
        recipientLower.contains('didi')) {
      return 'Transporte';
    }
    if (recipientLower.contains('restaurante') ||
        recipientLower.contains('comida') ||
        recipientLower.contains('pizza') ||
        recipientLower.contains('hamburguesa') ||
        recipientLower.contains('verduras') ||
        recipientLower.contains('vegetales') ||
        recipientLower.contains('mercado') ||
        recipientLower.contains('supermercado') ||
        recipientLower.contains('bodega')) {
      return 'Comida';
    }
    if (recipientLower.contains('farmacia') ||
        recipientLower.contains('medicina') ||
        recipientLower.contains('doctor')) {
      return 'Salud';
    }
    if (recipientLower.contains('cine') ||
        recipientLower.contains('pelicula') ||
        recipientLower.contains('entretenimiento')) {
      return 'Entretenimiento';
    }
    if (recipientLower.contains('curso') ||
        recipientLower.contains('libro') ||
        recipientLower.contains('educacion')) {
      return 'Educación';
    }
    if (recipientLower.contains('familia') ||
        recipientLower.contains('mama') ||
        recipientLower.contains('papa')) {
      return 'Familia';
    }
    if (recipientLower.contains('luz') ||
        recipientLower.contains('agua') ||
        recipientLower.contains('internet') ||
        recipientLower.contains('servicio')) {
      return 'Servicios';
    }
  ...
  static Future<String> _getSubcategory(
    String category,
    String recipient,
  ) async {
    final recipientLower = recipient.toLowerCase();
    ...
    switch (category) {
      case 'Transporte':
        ...
        if (recipientLower.contains('uber') ||
            recipientLower.contains('didi')) {
          return 'Uber/Didi';
        }
        return 'Otros';
      case 'Comida':
        ...
        if (recipientLower.contains('verduras') ||
            recipientLower.contains('vegetales')) {
          return 'Verduras y túbérculos';
        }
        if (recipientLower.contains('mercado') ||
            recipientLower.contains('supermercado') ||
            recipientLower.contains('bodega')) {
          return 'Supermercado';
        }
