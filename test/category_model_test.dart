...
void main() {
  ...
  test(
    'CategoryManager exposes the dynamic service as legacy values',
    () async {
      final db = AppDatabase.forTesting(NativeDatabase.memory());
      addTearDown(db.close);
      final repository = CategoryRepository(db);
      await repository.clearAllCategories();
      final inserted = await repository.addCategory(
        _category(createdAt, updatedAt),
      );
      expect(inserted, isNotNull);
      final service = CategoryService(repository);

      final category = await CategoryManager.getCategoryByName(
        service,
        'Comidas',
      );
      expect(category, isNotNull);
      expect(category!.name, 'Comidas');
      expect(category.icon, 'restaurant');
      expect(category.color, '#FF8800');
      expect(category.subcategories, ['Restaurantes', 'Delivery']);
      expect(await CategoryManager.getSubcategories(service, 'Comidas'), [
        'Restaurantes',
        'Delivery',
      ]);
      expect(
        await CategoryManager.getCategoryIcon(service, 'Comidas'),
        'restaurant',
      );
      expect(
        await CategoryManager.getCategoryColor(service, 'Comidas'),
        '#FF8800',
      );

      final all = await CategoryManager.getAllCategories(service);
      expect(all, hasLength(1));
      expect(all.single.name, 'Comidas');
      expect(
        await CategoryManager.getCategoryByName(service, 'Inexistente'),
        isNull,
      );
    },
  );
