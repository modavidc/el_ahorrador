...
void main() {
  group('TransactionTags regressions', () {
    testWidgets('renders nothing when no metadata is available', (
      tester,
    ) async {
    ...
    testWidgets('invalid confidence is displayed safely as zero', (
      tester,
    ) async {
      await tester.pumpWidget(
        _host(
  ...
  group('animated feedback regressions', () {
    testWidgets(
      'processing callback fires once after its configured duration',
      (tester) async {
        var calls = 0;
        await tester.pumpWidget(
          _host(
