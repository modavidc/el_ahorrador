...
void main() {
  group('CaptureValidator', () {
    test('clasifica proveedores conocidos sin depender de mayusculas', () {
      expect(CaptureValidator.validateCapture('YAPE - pago realizado'), CaptureType.yape);
      expect(CaptureValidator.validateCapture('BINANCE wallet'), CaptureType.binance);
      expect(CaptureValidator.validateCapture('Transferencia BBVA'), CaptureType.banco);
    });
    ...
    test('solo permite procesar tipos financieros reconocidos', () {
      for (final type in [CaptureType.yape, CaptureType.binance, CaptureType.banco]) {
...
''';
    ...
    test('rechazan texto ajeno al proveedor', () {
      ...
      expect(banco.BancoParser.parseBancoTransaction('recibo generico'), isNull);
      expect(binance.BinanceParser.parseBinanceTransaction('recibo generico'), isNull);
    });
  });

  group('Parser integrado', () {
    test('texto desconocido produce resultado seguro y respeta fallback', () async {
    ...
    test('propaga el resultado especializado de Binance', () async {
      ...
      expect(result.dateEpochMs, DateTime(2026, 7, 8, 14, 3, 59).millisecondsSinceEpoch);
